...
class RewardCron {
  ...
  async processMonthlyReward(istNow) {
    try {
      ...
      const firstDayThisMonth = new Date(istNow.getFullYear(), istNow.getMonth(), 1);
      const lastDayThisMonth = new Date(istNow.getFullYear(), istNow.getMonth() + 1, 0, 23, 59, 59, 999);

      console.log(`📅 Filtering users from ${firstDayThisMonth.toISOString()} to ${lastDayThisMonth.toISOString()}`);
      ...
      if (adminWallet.balance < totalPool) {
        console.log(`❌ Insufficient admin balance: Need ₹${totalPool}, available ₹${adminWallet.balance}`);
        return;
      }
      ...
      await WalletHistories.create({
        ...
        source: 'wallet',
        status: 'completed',
        note: `Monthly reward pool deducted for ${firstDayThisMonth.toLocaleString('default', { month: 'long' })}`,
      });
      ...
      for (const user of users) {
        ...
        await WalletHistories.create({
          ...
          source: 'wallet',
          status: 'completed',
          note: `Monthly reward for joining in ${firstDayThisMonth.toLocaleString('default', { month: 'long' })}`,
        });

        console.log(`✅ ₹${amountPerUser.toFixed(2)} sent to user ${user.userId}`);
      }
