...
class ServiceController {
  async createService(req, res) {
    try {
      ...
      const existingService = await ServiceRepository.findOneByType(
        serviceName
      );
      if (existingService) {
  ...
  async updateService(req, res) {
    ...
    const { serviceType, status, description, serviceIcon } = req.body;
    console.log(req.body);
    try {
      const updatedService = await ServiceRepository.findByIdAndUpdate(
        serviceId,
        { serviceType, status, description, serviceIcon },
        { new: true }
      );
  ...
  async deleteService(req, res) {
    ...
    try {
      const deletedService = await ServiceRepository.findByIdAndDelete(
        serviceId
      );
